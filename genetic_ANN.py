...
def fitness_function(chromosome):
    ...
    # Input layer
    model.add(tf.keras.layers.Dense(neurons_per_layer[0], input_shape=(1, INPUT_SIZE)))

    # Hidden layers
    for i in range(1, num_hidden_layers-1):
        model.add(tf.keras.layers.Dense(neurons_per_layer[i]))
    # Output layer
    model.add(tf.keras.layers.Dense(neurons_per_layer[num_hidden_layers-1]))
